...
log_report_status() {
    ...
    echo "[$timestamp] [$module] STATUS: $status" >> /var/log/logger_provision_report.log
}

# Check if command exists and services active
...
check_last_apt_update() {
    ...
    if [ ! -d /var/lib/apt/lists/ ]; then
        log_stage "[!] APT lists directory not found. Assuming no update was ever run."
        return 1
    fi
    ...
    if [ -z "$last_update_time" ]; then
        log_stage "[!] No apt list files found. Assuming no update was ever run."
        return 1
    fi
...
check_last_apt_upgrade() {
    ...
    if [ ! -f /var/log/apt/history.log ]; then
        log_stage "[!] No apt history log found. Assuming no upgrade was ever run."
        return 1
    fi
    ...
    if [ -z "$last_upgrade_time" ]; then
        log_stage "[!] No upgrade history found. Assuming never upgraded."
        return 1
    fi
    ...
    if [ -z "$last_upgrade_epoch" ]; then
        log_stage "[!] Could not parse upgrade timestamp. Assuming outdated."
        return 1
    fi
...
update_system_and_install_dependencies() {
    ...
    if ! check_last_apt_update; then
        ...
        log_stage "[*] Checking for apt-fast..."
        
        # Try to use apt-fast, otherwise fallback to apt
        if command -v apt-fast >/dev/null 2>&1; then
            APTFAST_CMD="apt-fast"
            log_stage "[+] apt-fast is already installed and will be used."
        else
            log_stage "[!] apt-fast not found. Attempting installation..."
            apt-get install -y apt-fast || {
                log_stage "[!] Failed to install apt-fast. Falling back to apt."
                APTFAST_CMD="apt"
            }
            
            # Check again if apt-fast was installed successfully
            if command -v apt-fast >/dev/null 2>&1; then
                APTFAST_CMD="apt-fast"
                log_stage "[+] apt-fast installed successfully and will be used."
            else
                APTFAST_CMD="apt"
                log_stage "[!] apt-fast is still unavailable. apt will be used instead."
            fi
        fi
        
        log_stage "[*] Selected package manager: $APTFAST_CMD"
        ...
        # Repositórios extras para versões até 22.04
        if [[ "$UBUNTU_VERSION" =~ ^(18\.04|20\.04|22\.04)$ ]]; then
            add-apt-repository -y ppa:apt-fast/stable || true
            add-apt-repository -y ppa:rmescandon/yq || true
            add-apt-repository -y ppa:oisf/suricata-stable || true
            apt-get update
        fi
        
        log_stage "Using ${APTFAST_CMD} to install base packages..."
        
        # Lista de pacotes básicos
        local packages=(
            yq crudini flex bison
            cmake make gcc g++ libpcap-dev libssl-dev python3-dev zlib1g-dev libcurl4-openssl-dev
            freerdp2-dev libpango1.0-dev libvncserver-dev libtelnet-dev
            libvorbis-dev libwebp-dev libssh2-1-dev libossp-uuid-dev
            mysql-server libcairo2-dev libjpeg-turbo8-dev libpng-dev
            libtool-bin libavcodec-dev libavutil-dev libswscale-dev
        )
        ${APTFAST_CMD} install --no-install-recommends -y "${packages[@]}" || true
        ...
        log_stage "Not needed update apt cache now. Cache is recent."
    fi
    
    if ! check_last_apt_upgrade; then
        log_stage "[*] Running apt-get upgrade..."
        apt-get upgrade -y
    else
        log_stage "[*] apt-get upgrade not needed. System is up to date."
    fi
}
...
install_openvswitch() {
    ...
    if is_installed ovs-vsctl && systemctl is-active --quiet openvswitch-switch; then
        log_stage "Open vSwitch already installed and running."
        SUCCESS+=("Open vSwitch is running.")
        return 0
    fi
    ...
    if command -v ovsdb-server >/dev/null 2>&1; then
        ...
        if systemctl is-active --quiet openvswitch-switch; then
            log_stage "[*] Open vSwitch installed and running."
            SUCCESS+=("Open vSwitch is active.")
        else
            log_stage "[!] Open vSwitch installed but not running."
            WARNING+=("OVS not running.")
        fi
...
## Configure Hostname and Network (Simplified - Initial config already done)
configure_hostname_and_network(){
    echo -e "\n==========================================================================="
    log_stage " [*] Verifying hostname and network settings..."
    echo -e "==========================================================================="
    ...
    if [ "$CURRENT_HOSTNAME" = "logger" ]; then
        ...
        SUCCESS+=("Hostname configured correctly")
    else
        log_stage "[!] Hostname not set correctly. Current: $CURRENT_HOSTNAME"
        log_stage "[+] Attempting to fix hostname..."
        hostnamectl set-hostname logger
        echo "logger" > /etc/hostname
        WARNING+=("Hostname was corrected")
    fi
    ...
    if [ "$ETH1_IP" = "192.168.56.105" ]; then
        ...
        log_stage "[!] eth1 IP not set correctly. Current: $ETH1_IP"
        WARNING+=("Network configuration may need manual adjustment")
    fi
...
verify_network_connectivity() {
    echo -e "\n==========================================================================="
    log_stage " [*] Verifying network connectivity..."
    echo -e "==========================================================================="
    
    # Install dnsutils if not present
    if ! command -v dig >/dev/null 2>&1; then
        log_stage "[+] Installing dnsutils..."
        apt-get install -y dnsutils
    fi
    
    # Test DNS resolution
    log_stage "[+] Testing DNS resolution..."
    if dig +short @8.8.8.8 github.com >/dev/null 2>&1; then
        log_stage "[+] DNS resolution working correctly"
        SUCCESS+=("DNS resolution verified")
    else
        log_stage "[X] DNS resolution failed"
        WARNING+=("DNS resolution issues detected")
    fi
    
    # Test connectivity to other VMs (if they exist)
    log_stage "[+] Testing connectivity to other lab VMs..."
    for vm_ip in "192.168.56.102" "192.168.56.103" "192.168.56.104"; do
        if ping -c 1 -W 2 "$vm_ip" >/dev/null 2>&1; then
            log_stage "[+] Connectivity to $vm_ip confirmed"
        else
            log_stage "[!] No connectivity to $vm_ip (VM may not be running yet)"
        fi
    done
...
fix_splunk_ssl_and_admin() {
    echo "[*] Fixing SSL certificates of the Splunk and creating admin user default..."
    ...
    echo "[*] Cleaning older certificates..."
    ...
    echo "[*] Generating the new certificate of the server..."
    ...
    if ! $SPLUNK_BIN status | grep -q "splunkd is running"; then
        echo "[X] Splunk is not running normally. Aborting this process..."
        return 1
    fi
    
    echo "[*] Checking Management API (8089)..."
    if curl -sk https://127.0.0.1:8089/services/server/info | grep -q "Unauthorized"; then
        echo "[*] Anyone user configured. Let's go creating user ($ADMIN_USER)..."
...
EOF
    ...
    # 9. Validação Final
    echo "[*] Validating final access to API..."
    if curl -sku "$ADMIN_USER:$ADMIN_PASS" https://127.0.0.1:8089/services/server/info | grep -q "<title>server-info</title>"; then
        echo "[✅] Splunk configured with success! Admin: $ADMIN_USER / Senha: $ADMIN_PASS"
    else
        echo "[X] There are a problem, finishing the configuration. Check the logs."
        return 1
    fi
}
...
install_splunk() {
    ...
    if [ -f "/opt/splunk/bin/splunk" ] || command -v splunk &>/dev/null; then
        ...
        log_stage "[!] Waiting for Splunk API availability..."
        for i in {1..90}; do
            if curl -sku admin:changeme https://127.0.0.1:8089/services/server/info | grep -q "<title>server-info</title>"; then
...
configure_splunk_apps_and_indexes() {
    log_stage "Creating Splunk indexes..."
    INDEXES=(wineventlog osquery osquery-status sysmon powershell zeek suricata threathunting evtx_attack_samples msexchange)
    
    for index in "${INDEXES[@]}"; do
        /opt/splunk/bin/splunk add index "$index" -auth "$SPLUNK_AUTH"
        log_stage "→ Index '$index' created."
    done
    
    log_stage "Installing core Splunk apps..."
    declare -A SPLUNK_APPS=(
        [windows]="splunk-add-on-for-microsoft-windows_700.tgz"
        [sysmon]="splunk-add-on-for-microsoft-sysmon_1062.tgz"
        [asn]="asn-lookup-generator_110.tgz"
        [editor]="lookup-file-editor_331.tgz"
        [zeek]="splunk-add-on-for-zeek-aka-bro_400.tgz"
        [viz1]="force-directed-app-for-splunk_200.tgz"
        [viz2]="punchcard-custom-visualization_130.tgz"
        [viz3]="sankey-diagram-custom-visualization_130.tgz"
        [link]="link-analysis-app-for-splunk_161.tgz"
        [hunt]="threathunting_1492.tgz"
    )
    
    for app in "${!SPLUNK_APPS[@]}"; do
        FILE="/vagrant/resources/splunk_server/${SPLUNK_APPS[$app]}"
        if [ -f "$FILE" ]; then
            /opt/splunk/bin/splunk install app "$FILE" -auth "$SPLUNK_AUTH"
            log_stage "→ App '$app' installed."
        else
            WARNING+=("App '$app' not found at $FILE")
        fi
    done
    
    log_stage "Applying minor app fixes..."
    ...
    # Force Splunk tour to be skipped
...
install_zeek_dependences() {
    apt-get update
    apt-get install -y cmake make gcc g++ flex bison libpcap-dev \
    libssl-dev python3-dev zlib1g-dev libcurl4-openssl-dev \
    libmaxminddb-dev libgeoip-dev liblzma-dev libmagic-dev \
    libreadline-dev libgoogle-perftools-dev libjemalloc-dev \
    binutils
}
...
install_zeek() {
    ...
    if [ "$(uname -m)" != "x86_64" ]; then
        log_stage "[X] Unsupported architecture: $(uname -m). Zeek supports only x86_64 builds here."
        ERROR+=("Zeek install failed: unsupported architecture.")
        return 1
    else
        if [ -f /opt/zeek/bin/zeek ] || [ -f /etc/systemd/system/zeek.service ]; then
            ...
            case "$UBUNTU_VERSION" in
                18.04|20.04)
                    ...
                    apt-get update && apt-get install -y zeek-lts && return 0
                ;;
                22.04|24.04)
                    ...
                    if ! ./configure --prefix=/opt/zeek; then
                        log_stage "[!] Configure failed. Trying to install via APT zeek-lts as fallback..."
                        echo "deb [signed-by=/usr/share/keyrings/zeek-archive-keyring.asc] http://download.opensuse.org/repositories/security:/zeek/xUbuntu_22.04/ /" > /etc/apt/sources.list.d/zeek.list
                        curl -fsSL "https://download.opensuse.org/repositories/security:/zeek/xUbuntu_22.04/Release.key" -o /usr/share/keyrings/zeek-archive-keyring.asc
                        apt-get update && apt-get install -y zeek-lts && return 0
                        log_stage "[X] Fallback install failed too."
                        return 1
                    fi
                    ...
                    if [ ! -x /opt/zeek/bin/zeek ]; then
                        log_stage "[X] Zeek binary not found after install. Aborting."
                        return 1
                    fi
                ;;
                *)
...
EOF
            ...
            for iface in eth0 eth1; do
                if ip link show "$iface" &>/dev/null; then
                    crudini --set $NODECFG "worker-$iface" type worker
                    crudini --set $NODECFG "worker-$iface" host localhost
                    crudini --set $NODECFG "worker-$iface" interface $iface
                    crudini --set $NODECFG "worker-$iface" lb_method pf_ring
                    crudini --set $NODECFG "worker-$iface" lb_procs "$(nproc)"
                fi
            done
            ...
            if ! pgrep -f zeek >/dev/null; then
                ...
                journalctl -u zeek --no-pager -n 20
                WARNING+=("Zeek failed to start.")
                FIXES+=("Try start Zeek again manually. Check systemd logs.")
                return 1
            fi
...
install_velociraptor() {
    ...
    DEB_FILE=$(ls velociraptor_*_server.deb 2>/dev/null | head -1)
    if [ -f "$DEB_FILE" ]; then
        dpkg -i "$DEB_FILE"
        echo "[$(date +%H:%M:%S)]: Installation complete!"
    else
        echo "[$(date +%H:%M:%S)]: Failed to find .deb file for installation."
        return 1
    fi
}
...
install_guacamole() {
    ...
    wget --progress=bar:force "https://archive.apache.org/dist/guacamole/1.3.0/source/guacamole-server-1.3.0.tar.gz" -O guacamole-server-1.3.0.tar.gz
    tar -xf guacamole-server-1.3.0.tar.gz && cd guacamole-server-1.3.0 || {
        echo "[-] Unable to find the Guacamole folder."
        return 1
    }
    
    echo "[$(date +%H:%M:%S)]: Compiling Guacamole with OpenSSL 3 compatibility..."
    CFLAGS="-Wno-error=deprecated-declarations" ./configure --with-init-dir=/etc/init.d
    make -j"$(nproc)"
    make install
    ldconfig
    
    # Detect Tomcat webapps dir
    TOMCAT_WEBAPPS=$(find /var/lib -type d -name webapps | grep tomcat | head -1)
    if [[ -z "$TOMCAT_WEBAPPS" ]]; then
        echo "[-] Could not detect Tomcat webapps folder. Aborting."
        return 1
    fi
    cd "$TOMCAT_WEBAPPS"
    
    wget --progress=bar:force "https://archive.apache.org/dist/guacamole/1.3.0/binary/guacamole-1.3.0.war" -O guacamole.war
    
    mkdir -p /etc/guacamole/shares
    chmod 777 /etc/guacamole/shares
    
    GUACAMOLE_USER_DIR=$(find /usr/share -type d -name ".guacamole" | grep tomcat | head -1)
    if [[ -z "$GUACAMOLE_USER_DIR" ]]; then
        GUACAMOLE_USER_DIR="/usr/share/$(basename "$TOMCAT_WEBAPPS")/.guacamole"
        mkdir -p "$GUACAMOLE_USER_DIR"
    fi
    ...
    ln -sf /etc/guacamole/guacamole.properties "$GUACAMOLE_USER_DIR/"
    ln -sf /etc/guacamole/user-mapping.xml "$GUACAMOLE_USER_DIR/"
    
    if ! id -u guacd &>/dev/null; then
    ...
    # Enable correct tomcat version
    if systemctl list-units --type=service | grep -q tomcat10; then
        systemctl enable tomcat10
        systemctl start tomcat10
        elif systemctl list-units --type=service | grep -q tomcat9; then
        systemctl enable tomcat9
        systemctl start tomcat9
    else
        echo "[!] Could not detect tomcat9 or tomcat10 service."
    fi
    
    systemctl start guacd
    
    echo "[$(date +%H:%M:%S)]: Guacamole installation complete!"
    SUCCESS+=("Guacamole is installed.")
}
...
configure_splunk_inputs() {
    ...
    log_stage "[*] Restarting Splunk service to apply all configurations..."
    /opt/splunk/bin/splunk restart
}
...
main() {
    ...
    for module in "${MODULES[@]}"; do
        if $module; then
            ...
            log_report_status "$module" "SUCCESS"
        else
            echo "❌ $module failed. Continuing with next steps..."
            log_report_status "$module" "FAILED"
...
ALLOWED_MODES=("main" "splunk_only" "velociraptor_only" "guacamole_only" "suricata_only" "zeek_only" "fleet_only" "fix_network_only" "cleanup_system")
